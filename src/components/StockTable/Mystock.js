...
class Mystock extends Component {
  ...
  componentDidMount() {
    axios
      .get(`https://financial-portfolio-7b98c.firebaseio.com/myStocks.json`)
      .then((db) => {
        const res = db.data;
        if (res === null) {
          this.setState({
          ...
          for (let i = 0; i < Keys.length; i++) {
            ...
            this.StockInfo = (x) => {
              axios
                .get(
                  `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${c_symb}&apikey=R1NCP7DHTAMQZ9G1`
                )
                .then((res) => {
                  let Data = [];
                  let Current_p = [];
